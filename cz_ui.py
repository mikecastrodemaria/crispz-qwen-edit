...
PRESETS = {
    ...
    # verdict du test VRAM 'auto' et gelait les cartes limites (spill RAM partagee).
...
def _ui_set_text_encoder(src):
    """Applique et memorise l'encodeur choisi (pipes base ET edition) -- sauf s'il ne
    convient pas au repo de base: refus nomme, rien de change, rien d'ecrit dans les
    preferences. Le pipe d'edition refait sa propre verification a son chargement."""
    src = (src or "").strip()
    ...
    reload = (" Base and edit pipes reload on next run; the prompt cache was cleared."
              if changed else "")
    if not src:
        return "Text encoder: each repo's own." + reload
    return (f"Text encoder: **{cz_pipeline._encoder_label(src)}** on the base and edit "
...
def _vram_hint(e):
    ...
    return ("  \n**VRAM full**, even after clearing the cache: close the other GPU apps "
            ...
            "references. If the next render fails too, restart crispz-qwen-edit.")
    ...
    try:
        ...
        if use_input and input_mode == "Reference (Omni)" and omni_ready:
            ...
            for i in range(n):
                ...
                picked_styles = _pick_styles(styles, style_random)
                base_prompt, base_negative = expand_prompt_pair(prompt, negative, s, index=i)
                full_prompt, full_negative = _apply_styles(base_prompt, base_negative,
                                                           picked_styles)
                if style_random:
                ...
                try:
                    ...
                    if "CUDA" in str(e) or "out of memory" in str(e).lower():
                        msg += ("  \n**VRAM full** (another GPU app like ComfyUI still loaded? "
                                ...
                                "crispz-qwen-edit** (the CUDA context is dead), lower refine_tile / factor.")
                    ...
                    with gr.Tab("Models"):
                        ...
                        with gr.Accordion("ℹ️  What is CPU offload?", open=False):
                            gr.Markdown(
                                ...
                                "otherwise `model`/`sequential`. Qwen-Image bf16 is ~57 GB all-in (transformer "
                                "~40 + encoder ~17), so `auto` resolves to `model` on consumer cards (a GGUF is "
                                "forced to `model` anyway). The verdict is cached per GPU + model "
                                "(cache/hw_profile.json) — use **Re-test VRAM** after closing/opening another "
                                "GPU app.\n"
                                "- **none** — everything stays in VRAM. **Fastest**, needs the full footprint free.\n"
                        ...
                        with gr.Accordion("\U0001F4E6 Checkpoints (switch model)", open=True):
                            ...
                            with gr.Column():
                                te_dd = gr.Dropdown(
                                    ...
                                    info="Default: each repo's own Qwen2.5-VL-7B. Or a transformers "
                                         "folder (config.json + weights) or HF repo of the SAME "
                                         "size, e.g. an abliterated Qwen2.5-VL-7B. Used by the base "
                                         "AND edit pipes. Pick or paste a path; changing it reloads "
